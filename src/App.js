...
function App() {
  const [simpsonOnly, setsimpsonOnly] = React.useState(false);
  ...
  function makeSimpsonOnly() {
    setsimpsonOnly(!simpsonOnly);
  }

  return (
    <div>
      <button className="button" onClick={makeSimpsonOnly}>
        Switch Simpson's Family: {simpsonOnly ? 'ON' : 'OFF'}
      </button>
      {quoteList
        .filter(
          (members) => !simpsonOnly || members.character.includes('Simpson')
        )
        .map((quote, index) => (
